...
import { connect, subscribeToUserQueue, disconnect, editMessage, sendMessage, deleteMessage } from './WebSocketService';
...
const WebSocketComponent = () => {
    ...
    useEffect(() => {
        setUserOptions([
            { value: '8d5e2971-66e1-47f1-a1ee-0ae9c2142ae8', label: 'john@example.com' },
            { value: '8a5c1b3f-0e78-43b6-951d-f5a4e8d5c0b3', label: 'alice@example.com' }
        ]);
    }, []);
        ...
        const onMessageReceived = message => {
            ...
            if (body.content) {
                setMessages(prevMessages => {
                    const existingMessageIndex = prevMessages.findIndex(msg => msg.id === body.id);
                ...
                setMessages(prevMessages => prevMessages.filter(msg => msg.id !== body.messageId));
            }
        };

        if (accessToken) {
            connect(accessToken, onMessageReceived);
        }
    ...
    const handleSendMessage = () => {
        if (accessToken && newMessageContent) {
    ...
    const handleEditMessage = (messageId, newContent) => {
        if (accessToken && messageId && newContent) {
            ...
            console.error('Access token, message ID, and new content are required.');
        }
    };
        ...
        <div>
            ...
            <ul>
                {messages.map((msg) => (
                    <li key={msg.id}>
                        {msg.content}
                        <button onClick={() => handleEditMessage(msg.id, 'Updated content')}>Edit</button>
            ...
            <div>
                ...
                />
                <button onClick={handleConnectToUserQueue}>Connect to User Queue</button>
